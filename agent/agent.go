...
func NewAgentServer(ctx *g.Context, opts ...ServerOption) *TCPService {
	...
	tcp.client = NewClient(ctx.Ctx, SetOnMessage(tcp.onClientMessage))
	// 服务注册
	...
	tcp.server = NewServer(ctx.Ctx, cfg.Listen, SetOnServerMessage(tcp.onServerMessage))
	return tcp
}
...
func (tcp *TCPService) onClientMessage(client *Client, content []byte) {
...
func (tcp *TCPService) onServerMessage(node *ClientNode, msgID int64, data []byte) {
}
