...
import (
	...
	wstring "github.com/mia0x75/centineld/string"
	"math/big"
)
...
func (h *binlogHandler) append(buf *[]byte, edata interface{}, column *schema.TableColumn) {
	log.Debugf("Handler::append  - %+v,===,%+v, == %+v", column, reflect.TypeOf(edata), edata)
	switch edata.(type) {
	case string:
...
func (h *binlogHandler) OnRow(e *canal.RowsEvent) error {
	...
	log.Debugf("Handler::OnRow - binlog缓冲区详细信息: %d %d", len(h.buf), cap(h.buf))
		...
		for i := 0; i < len(e.Rows); i += 2 {
			...
			for k, col := range e.Table.Columns {
				...
				if k < rows_len {
					...
					log.Warnf("Column: %+v", &col)
					log.Warnf("Unkonw data type, db: %s, table: %s, column: %s. ", e.Table.Schema, e.Table.Name, col.Name)
					edata = nil
				}
...
func (h *binlogHandler) getBinlogPositionCache() (string, int64, int64) {
	wfile := file.WFile{"/run/centineld/centineld.cache"}
	str := wfile.ReadAll()
	if str == "" {
